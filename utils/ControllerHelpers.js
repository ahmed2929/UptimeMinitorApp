...
const getMedInfoFromFhirPrescription = async (Prescription) => {
  ...
  try {
    ...
    medInfo.Name = Prescription.contained ? Prescription.contained[0] ? Prescription.contained[0].code.coding[0].display : '' : ''
    medInfo.MedID = Prescription.contained ? Prescription.contained[0] ? Prescription.contained[0].id : '' : ''
    medInfo.Note = Prescription.note ? Prescription.note[0] ? Prescription.note[0].text : '' : ''
    ...
    medInfo.quantity = Prescription.dispenseRequest ? Prescription.dispenseRequest.quantity ? Prescription.dispenseRequest.quantity.value : '' : ''
    medInfo.unit = Prescription.dispenseRequest ? Prescription.dispenseRequest.quantity ? Prescription.dispenseRequest.quantity.unit : '' : ''
    medInfo.reason = Prescription.reason ? Prescription.reason[0].concept.coding[0].display : ''
    return medInfo
...
const getDoseInfoFromFhirPrescription = async (Prescription) => {
  ...
  try {
    ...
    Prescription.dosageInstruction.forEach((dose) => {
      ...
      DoseInfo.rateQuantity = dose.doseAndRate ? dose.doseAndRate[0].doseQuantity ? dose.doseAndRate[0].doseQuantity.value : '' : ''
      DoseInfo.rateUnit = dose.doseAndRate ? dose.doseAndRate[0].doseQuantity ? dose.doseAndRate[0].doseQuantity.unit : '' : ''
      DoseInfo.rateRange = dose.doseAndRate ? dose.doseAndRate[0].doseRange ? dose.doseAndRate[0].doseRange.low.value : '' : ''
      DoseInfo.rateRangeUnit = dose.doseAndRate ? dose.doseAndRate[0].doseRange ? dose.doseAndRate[0].doseRange.low.unit : '' : ''
...
const createMedicationFromFhirPrescription = async (medInfo, doseInfo, ProfileID) => {
  try {
...
const createSchedulerFromFhirPrescription = async (doseInfo, MedicationID, ProfileID) => {
  try {
    ...
    doseInfo.forEach((dose) => {
      ...
      if (dose.boundsPeriod) {
        ...
        startDate = new Date()
        endDate = new Date()
        endDate.setDate(startDate.getDate() + dose.boundsDuration.value)
      } else {
        startDate = new Date()
        endDate = new Date()
      ...
      SchedulerArray.push(newScheduler)
    })
